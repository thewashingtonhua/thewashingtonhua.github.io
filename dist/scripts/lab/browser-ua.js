window.onload = function () {
  if (navigator.userAgent && navigator.platform) {
    ...
    document.getElementById('ua').innerHTML = UAString;
...
function getBrowser() {
  ...
  if (ua.indexOf('edge') > -1) {
    ...
    // IE 11  only have 'Trident', IE 8,9,10   have both 'MSIE' & 'Trident', IE 6,7 only have 'MSIE'
    browser = 'IE 11';
    ...
    if (Number(browserVersion) >= 9.8) {
      // Opera 10+
      browserVersion = ua.match(/version\/\d+(.\d+)+/)[0].substr(8);
    }
    if (ua.indexOf('mini') > -1) {
      // Opera mini
    ...
    if (ua.indexOf('blackberry') > -1 || ua.indexOf('bb10') > -1 || ua.indexOf('playbook') > -1) {
      var webkitVersion = ua.match(/webkit\/\d+(.\d+)+/)[0].substr(7);
...
function otherBrowserCheck(browser) {
  ...
  if (ua.indexOf('micromessenger') > -1) {
    var weixinVersion = ua.match(/micromessenger\/\d+(.\d+)+/)[0].substr(15);
  ...
  // QQ
  if (ua.indexOf('qq/') > -1) {
    var qqVersion = ua.match(/qq\/\d+(.\d+)+/)[0].substr(3);
    webkitVersion = ua.match(/webkit\/\d+(.\d+)+/)[0].substr(7);
  ...
  if (ua.indexOf('ucbrowser') > -1 || ua.indexOf(' ubrowser') > -1) {
    ...
    if (ua.indexOf('mobile') > -1) {
      ...
      if (ua.indexOf('chrome') > -1) {
        ...
        var edgeVersion = ua.match(/edge\/\d+(.\d+)+/)[0].substr(5);
        browser = 'UC Browser ' + browserVersion + ' / edge ' + edgeVersion;
      } else if (ua.indexOf('msie') > -1) {
        var ieVersion = ua.match(/msie\s\d+/)[0].substr(5);
  ...
  if (ua.indexOf('sogou') > -1 || ua.indexOf(' se ') > -1) {
    if (ua.indexOf('mobile') > -1) {
      ...
      if (ua.indexOf('chrome') > -1) {
        ...
        var _edgeVersion = ua.match(/edge\/\d+(.\d+)+/)[0].substr(5);
        browser = 'Sogou Browser / edge ' + _edgeVersion;
      } else if (ua.indexOf('msie') > -1) {
        var _ieVersion = ua.match(/msie\s\d+/)[0].substr(5);
        browser = 'Sogou Browser / IE ' + _ieVersion;
      } else if (ua.indexOf('trident') > -1) {
  ...
  if (ua.indexOf('bidu') > -1) {
    if (ua.indexOf('chrome') > -1) {
      ...
      var _edgeVersion2 = ua.match(/edge\/\d+(.\d+)+/)[0].substr(5);
      browser = 'Baidu Browser ' + browserVersion + ' / edge ' + _edgeVersion2;
    } else if (ua.indexOf('msie') > -1) {
      browserVersion = ua.match(/browser\s\d+(.\d+)+/)[0].substr(8);
      var _ieVersion2 = ua.match(/msie\s\d+/)[0].substr(5);
      browser = 'Baidu Browser ' + browserVersion + ' / IE ' + _ieVersion2;
    } else if (ua.indexOf('trident') > -1) {
      browserVersion = ua.match(/browser\s\d+(.\d+)+/)[0].substr(8);
...
function getOS() {
  ...
  if (window.navigator.userAgent && window.navigator.platform) {
    ...
    if (isMac) {
      os = 'Mac';
      var macVersionStr = ua.match(/mac\sos\sx\s[0-9]{1,2}(_[0-9]{1,2})+/);
      if (macVersionStr && macVersionStr.length) {
        var macVersion = macVersionStr[0].replace(/mac\sos\sx\s/, '').replace(/_/g, '.');
        os = os + ' ' + macVersion;
      }
      return os;
    }
    ...
    if (isIOS) {
      ...
      if (isIPhone) {
        ...
        var _reg = /cpu\sos\s\d+(_\d+)+/;
        var _str = ua.match(_reg);
        if (_str[0].length) {
          iosVersion = _str[0].substr(6).replace(/_/g, '.');
        }
        os = 'iOS ' + iosVersion + ' / iPad';
      } else if (isIPod) {
        var _reg2 = /cpu\siphone\sos\s\d+(_\d+)+/;
        var _str2 = ua.match(_reg2);
        if (_str2[0].length) {
          iosVersion = _str2[0].substr(6).replace(/_/g, '.');
        }
    ...
    if (isWinPhone) {
      var _reg3 = /\d+(.\d)+/;
      var _str3 = ua.substr(ua.indexOf('windows phone') + 14, 7);
      var winPhoneVersion = _reg3.exec(_str3)[0];
...
//# sourceMappingURL=browser-ua.js.map
