...
<img src='../img//illustration/electron-install-guide/code-install.svg' alt='' width='100%' >
...
<p>先执行常规的 npm 安装。</p>
<p>当要执行到<span class="code">node install.js</span>的时候，赶紧按下<span class="code">Ctrl + C</span>结束掉进程。别担心，到这里为止我们需要的内容已经都拿到了，剩下的我们手动来解决。（这一步的时机可能需要多试几次才能掐准，有点像 Windows 开机进 BIOS 的感觉，早了必要的文件还没下载完，晚了就错过了。实践发现报错的同时会把已经下载的内容连同 electron 目录一起删掉，按照报错信息会找不到文件，所以时机很重要）</p>
...
<blockquote>2018-10-06：<br/>
Electron 3.0 对 path 的取值做了些许调整，改成了“Electron.app/Contents/MacOS/Electron”</blockquote>
...
<p>比较意外的是，当我某一次尝试 cnpm 的时候，竟然奇迹般的一边安装成功，但使用 yarn 和 npm 时从未成功过（日常一直都是用的淘宝的源，为测试也用过官方的源，但都不行）。</p>