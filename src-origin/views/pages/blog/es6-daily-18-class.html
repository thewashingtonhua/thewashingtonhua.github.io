...
<img src='/img/illustration/es6-daily/class-private-property.png' alt=''>
...
<p>这个属性用在构造函数中（无论是 class 还是 function），返回<span class='code'>new</span>命令作用于的那个构造函数，如果构造函数不是通过<span class='code'>new</span>调用的，<span class='code'>new.target</span>会返回<span class='code'>undefined</span>。</p>
<p>这个属性可以用来确定构造函数是怎么调用的，比如强制构造函数只能用<span class='code'>new</span>调用，或者强制某各类不能直接实例化，必须继承后才可实例化。（子类继承父类时，<span class='code'>new.target</span>返回子类）</p>
<img src='/img/illustration/es6-daily/class-new-target.svg' alt=''>

<h2>继承</h2>
<p>Class 提供了<span class='code'>extends</span>关键字用于实现继承，相比修改原型链的写法，新语法更直观方便。</p>
<img src='/img/illustration/es6-daily/class-extends.svg' alt=''>
<p>子类构造函数中必须首先调用<span class='code'>super()</span>，否则子类无法得到<span class='code'>this</span>。子类自己的<span class='code'>this</span>依赖于父类的<span class='code'>this</span>，需要先得到父类同样的实例属性和方法，然后再覆盖以自己的实例属性和方法。</p>
<p>子类可以省略构造函数，这个方法会被默认添加，并传入和父类构造函数相同的参数，等于直接取得父类中定义的内容。如果你需要自定义子类的实例化方式，就需要遵循上面的步骤，手动调用<span class='code'>super()</span>。</p>

<h2>继承机制</h2>
<p>ES5 中，本质是先构造子类的<span class='code'>this</span>，再添加父类的属性（<span class='code'>Parent.apply(this)</span>），最后添加子类的属性。</p>
<p>ES6 中，是先通过父类获取<span class='code'>this</span>，再添加子类的属性，所以子类构造函数必须先调用<span class='code'>super()</span>才能正常访问<span class='code'>this</span>。</p>

<h2>Object.getPrototypeOf()</h2>
<p><span class='code'>Object.getPrototypeOf()</span>用于获取一个子类的父类、（你或许见过用<span class='code'>__proto__</span>来获取原型的方式，这是现代浏览器自己实现的快捷方式，不属于语言标准，Node.js 也不支持这样的语法，不建议使用）</p>
<img src='/img/illustration/es6-daily/class-getprototypeof.svg' alt=''>

<h2>super</h2>
<p><span class='code'>super</span>既可以作为函数（用在构造函数中，代表父类的构造函数，返回父类实例），也可以作为对象（用在子类实例方法中，指向父类的原型对象；用在子类静态方法中，指向父类）。</p>
<p>当<span class='code'>super</span>用作函数时，虽然它代表父类的构造函数，但返回的是子类的实例，即<span class='code'>super</span>内部的<span class='code'>this</span>指的是子类，因此<span class='code'>super</span>相当于<span class='code'>Parent.prototype.constructor.call(this)</span>。如果在父类的构造函数里执行<span class='code'>console.log(new.target.name)</span>，则它的子类在执行<span class='code'>super()</span>时这段代码会打出子类的构造函数，而不是父类。</p>
<p>当<span class='code'>super</span>出现在子类的实例方法中时，由于它指向父类的原型对象，因此只有定义在父类原型上的属性和方法才能被<span class='code'>super</span>访问到（<span class='code'>Parent.prototype.XXX</span>），定义在父类实例上的属性和方法<span class='code'>super</span>是访问不到的。子类实例方法在通过<span class='code'>super</span>调用父类方法时，方法内部的<span class='code'>this</span>指向当前子类实例（<span class='code'>super.method.call(this)</span>），如果通过<span class='code'>super</span>对某个属性赋值（不太能想象什么场景下会这么做，因此还是避免这种骚操作比较好），这时的<span class='code'>super</span>就是<span class='code'>this</span>，即该属性被认为是子类的实例属性。通过这种方式定义的属性，在后续的代码中可以通过<span class='code'>this.XXX</span>访问到，但如果访问<span class='code'>super.XXX</span>，实际访问的是<span class='code'>Parent.prototype.XXX</span>。</p>
<p>当<span class='code'>spuer</span>出现在子类的静态方法中时，由于它指向父类，因此只能访问到静态的属性和方法。子类静态方法中通过<span class='code'>super</span>调用父类方法时，方法内部的<span class='code'>this</span>指向子类，而不是子类的实例。</p>
<p><span class='code'>super</span>在使，用时必须显式地指定是作为函数还是对象使用，否则会报错（比如<span class='code'>console.log(super)</span>）。一般情况下，在构造函数中用作函数，在方法中用作对象，应该是遇不到这类问题的。</p>
<p>由于 JavaScript 中的对象总是继承自其他对象的，所以可以在任何对象中使用<span class='code'>super</span>关键字。</p>
...
<p>下一篇：<a href="./es6-daily-19-module.html">每天一点ES6(19)：Module</a></p>